...
EnergyMonitor -- Energy monitor configuration
 ├─> vue: PyEmVue (Emporia Vue Gen 3)
 ├─> channels: VueDeviceChannel (multiple)
...
Config -- EVBillingConfiguration and settings
...
from evbilltext import extract_text
...
class Bill:
    ...
    def __init__(self, ev_files: EVFiles, pdfpages: tuple[str, str]) -> None:
        ...
        self.ev_files: EVFiles = ev_files
        """EVFiles instance."""
        self.bill_path: Path = ev_files.bill_path
        """Path to the PG&E bill PDF file."""
        self.outdir: Path = ev_files.outdir
    ...
    def submeter_bill(self, submeter: 'Submeter') -> 'Bill':
        ...
        Notes
        -----
        This method creates a submeter bill by copying the main bill and updating it with
        the EV charger power and usage data.  Submeter bills have the same structure, line
        items, dates, and rates as the main bill.
    ...
    def valid(self) -> bool:
        ...
        # Check for PG&E total usage equal to PDF Total Usage
        if self.total_usage is not None:
            if self.pge_bill_details.total_kWh != self.total_usage:
                warning_msg(
                    ...
                    f'not equal to PDF total usage {self.total_usage}',
                )
                ok = False
...
class RatePeriod(ABC):
    ...
    def __init__(self, bill: Bill, period: BillPeriod, text: str) -> None:
        """Initialize a PG&E or CPSF rate period from PG&E bill PDF text.
       ...
       """
        ...
        """other_charges[name] = OtherEnergyCharge or OtherPctNetCharge instance."""
        ...
        for pattern, charge_class in self.other_charges_REs:
            if m := re.search(pattern, text, flags=re.IGNORECASE):
                ...
                warning_msg(
                    logger,
                    f'Meter {self.bill.meter_no} {self.provider} '
                    f'rate period {self.period} {name}: charge not found; assuming $0.00.',
                )
...
class PGERatePeriod(RatePeriod):
    ...
    def __init__(self, bill: Bill, period: BillPeriod, text: str) -> None:
        """Initialize PG&E rate period from PG&E bill PDF text.
    ...
    def other_charges_REs(self) -> tuple[tuple[str, type['OtherCharge']], ...]:
        """Return tuple of REs for instantiating OtherCharge instances."""
        return (
            (rf'(Franchise Fee Surcharge) {RE_DOLLARS}', OtherEnergyCharge),
    ...
    def valid(self) -> bool:
        ...
        if not self.subscription.valid():
            error_msg(logger, f'PG&E rate period {self.period} invalid {self.subscription}.')
            ok = False

        if self.overage_fee.charge == Dollars('0.00') and self.overage_fee.kW > Kilowatts('0.00'):
            if self.bill.is_main_bill:
                warning_msg(
                    logger,
                    f'PG&E rate period {self.period} No Overage Fees for '
                    f'{self.overage_fee.kW} kW overage, OK if Grace Period in effect.',
                )
        elif not self.overage_fee.valid():
...
class CPSFRatePeriod(RatePeriod):
    ...
    def __init__(self, bill: Bill, period: BillPeriod, text: str) -> None:
        """Initialize CleanPowerSF rate period from PG&E bill PDF text.
...
class BillDetails:
    ...
    def __init__(self, bill: Bill, text: str) -> None:
        ...
        bill : Bill
            Main or submeter Bill instance.
        text : str
            PDF text of PG&E Electric Delivery or CleanPowerSF
        ...
        """Main or submeter Bill instance."""
        self.text: str = text
        """PDF text of PG&E or CPSF page of bill."""
        self.total_charges_pdf: Dollars
        """PDF Total PG&E Electric Delivery or CleanPowerSF Electric charge."""
        ...
        self.total_charges_pdf = Dollars(m.group(1))
    ...
    def submeter_bill_details(self, submeter: 'Submeter') -> None:
        ...
        self.total_charges_pdf = (
            self.total_charges
        )  # No PDF for submeter bills; suppress errors from valid()
    ...
    def valid(self) -> bool:
        """Check validity of bill details, which can be incorrect due to PDF errors
        ...
        if self.total_charges != self.total_charges_pdf:
            error_msg(
                logger,
                f'Calculated {self.provider} total charges ${self.total_charges} '
                f'not equal to PDF total charges ${self.total_charges_pdf}.',
            )
            ok = False
...
class CPSFBillDetails(BillDetails):
    ...
    def __init__(self, bill: Bill, text: str) -> None:
        """Initialize CleanPowerSF Electric Generation bill details from extracted text.
        ...
        bill : Bill
            Bill instance
        text : str
            PDF text of CleanPowerSF Electric Generation bill details page.
        ...
        tou_labels: list[list[str]] = [
            re.findall(
                rf'({tou_label})\D+' rf'{RE_kWh} kWh @ {RE_RATES} {RE_DOLLARS}', text, re.IGNORECASE
            )
            for tou_label in (
                'Generation - On Peak',
                'Generation - Off Peak',
                'Generation - Super Off Peak',
            )
        ]

        if all(len(tou_label) == 1 for tou_label in tou_labels):
            return  # No rate change, no split needed

        if any(len(tou_label) != 2 for tou_label in tou_labels):
            raise LookupError(
                'Inconsistent CleanPowerSF energy charges found in bill details; check bill.',
            )

        if len(bill.pge_bill_details.rate_periods) != 2:
            raise LookupError(
                'No PG&E rate period found for corresponding CleanPowerSF rate change; '
                'check bill.',
            )
        ...
        if len(self.rate_periods) > 1:
            warning_msg(
                ...
                f'that includes rate change on {change_date} -- check bill.',
            )
            for rate_period in self.rate_periods:
        ...
        logger.info(f'Splitting CleanPowerSF rate period {self.period} on {change_date} ...')
        ...
        for rates in (old_rates, new_rates):
            other_charges: dict[str, OtherCharge] = rates.other_charges

            for name in ('Local Utility Users Tax', 'Energy Commission Surcharge'):
                if name in other_charges:
                    other_charges[name].set_charge(rates)
    ...
    def re_total_charges(self) -> str:
        """Return RE to parse total charges."""
        return rf'Total CleanPowerSF Electric Generation Charges \${RE_DOLLARS}'
...
class SubscriptionCharge:
    ...
    def __init__(self, quantity: str, months: str, rate: str, charge: str) -> None:
        """Initialize a SubscriptionCharge instance from PG&E bill PDF text.
        ...
        """Number of 10 kW blocks."""
        self.months = SubscriptionMonths(months)
        """Fraction of billing period from PG&E pdf."""
    ...
    def valid(self) -> bool:
        ...
        inaccurate, so validation compares the calculated amount to the PDF amount
        to within +- 0.01.

        """
        return abs(Dollars(self.months * self.quantity * self.rate) - self.charge) <= Dollars(
            '0.01'
        )
...
class OverageFee:
    ...
    def submeter_overage_fee(self, evse_kW_rating: Kilowatts, total_evse_kW: Kilowatts) -> None:
        ...
        portion: float = evse_kW_rating / total_evse_kW
        self.kW *= portion
        self.charge *= portion
...
class OtherCharge(ABC):
    ...
    Other PG&E credits, surcharges, and taxes:
        ...
        Franchise Fee Surcharge ($/Total kWh)
    ...
    def __init__(self, name: str, charge='0.00') -> None:
        """Initialize an OtherCharge instance from PG&E bill PDF text.
...
class OtherEnergyCharge(OtherCharge):
    """Other charge based on total energy (kWh) usage:

    PG&E Power Charge Indifference Adjustment ($/Total kWh)
    PG&E Franchise Fee Surcharge ($/Total kWh)
    CPSF Energy Commission Surcharge ($/Total kWh)

    """

    def __init__(self, name: str, charge='0.00') -> None:
        """Initialize an OtherEnergyCharge instance from PG&E bill PDF text.
        ...
        self.rate: DollarsPerKilowattHour = DollarsPerKilowattHour('NaN')
        """Calculated $/kWh rate."""
...
class OtherPctNetCharge(OtherCharge):
    ...
    def __init__(self, name: str, charge='0.00') -> None:
        """Initialize an OtherPctNetCharge instance from PG&E bill PDF text.
        ...
        """
        super().__init__(name, charge)
        self.rate: Percent = Percent('NaN')
        """Calculated % net charges rate."""
...
class Submeter:
    ...
    def __init__(self, main_bill: Bill, energy_monitor: EnergyMonitor, evse_id: str) -> None:
        ...
        """EVSE (EV charger) id."""
        self.evse_kW_rating: Kilowatts = energy_monitor.chargers[evse_id].kW
        """EV charger power rating in kW."""
        self.total_evse_kW: Kilowatts = energy_monitor.total_evse_kW
        """Total connected EV charger power ratings in kW."""
        self.hourly_kWh: npt.NDArray[np.float32] = cast(
            npt.NDArray[np.float32],
            energy_monitor.download_usage_data(self.evse_id, self.main_bill.period, zero_fill=True),
        )
        ...
        self.total_kWh: KilowattHours = KilowattHours(str(self.hourly_kWh.sum()))
        """Total energy usage."""
        ...
        self.tou_rates: npt.NDArray[np.float32] = main_bill.tou_rates()
        """tou_rates[Provider, day, hour] = rate ($/hourly_kWh)."""
        self.hourly_kWh_cost: npt.NDArray[np.float32] = self.hourly_kWh * self.tou_rates
        ...
        """Monthly history start date of first day of month."""
        self.monthly_tou_kWh_history: dict[Tou, npt.NDArray[np.float32]] = {
        ...
        service_start_date: date = energy_monitor.chargers[self.evse_id].on_dt
        end_date: date = (
            self.period.to
            if (self.period.to + ONE_DAY).day == 1  # last day of month
            else self.period.to.replace(day=1) - ONE_DAY
        )  # last day of previous month
        self.monthly_history_start_date: date = (end_date - 12 * 31 * ONE_DAY).replace(day=1)
        month: int = 0
        """Month counter for monthly history."""
        for month in range(13):
            first_of_month: date = date(end_date.year, end_date.month, 1)
            start_date: date = max(first_of_month, service_start_date)
            if start_date > end_date:
                break  # Ignore usage data prior to service start date

            period = BillPeriod(start_date.strftime('%m/%d/%Y'), end_date.strftime('%m/%d/%Y'))
            usage: npt.NDArray[np.float32] | None = energy_monitor.download_usage_data(
                self.evse_id, period
            )
            if usage is None:  # Channel contained no data
                warning_msg(
                    logger,
                    f'Missing usage data for EV charger {self.evse_id} '
                    f'from {start_date} to {end_date}.',
                )
                break

            for tou in Tou:
                self.monthly_tou_kWh_history[tou][-1 - month] = usage[
                    :, Config.tou_hours_mask[tou]
                ].sum()

            end_date = first_of_month - ONE_DAY
        ...
        with np.printoptions(precision=2, floatmode='fixed'):
            logger.info(
                ...
                f'starting from {max(self.monthly_history_start_date, service_start_date)}:\n'
                f'{self.monthly_tou_kWh_history}'
            )

        self.submeter_bill: Bill = main_bill.submeter_bill(
            self
    ...
    def dump_usage_and_costs(self, csvfile) -> None:
        """Dump submeter raw usage data and costs to .CSV file.
        ...
        total = subscription + super_off_peak + off_peak + peak

        cost_cols = np.column_stack(
            (
                subscription,
                super_off_peak,
                off_peak,
        ...
        combined_matrix = np.hstack((date_strings, self.hourly_kWh.astype(str), cost_cols))
        ...
        np.savetxt(
            ...
            + ','
            + 'Subscription $,'
            + ','.join([f'{tou} $' for tou in [Tou.SUPER_OFF_PEAK, Tou.OFF_PEAK, Tou.PEAK]])
            + ','
            + 'Total $',
        )
...
def process_main_bill(bill_path: Path) -> Bill:
    ...
    ValueError
        ...
        PG&E bill Statement Date is not a valid date
        PG&E bill Due Date is not a valid date
        Invalid PG&E Account Number
        PG&E bill file name does not match Account Number read from statement
        PG&E bill file name does not match Statement Date read from statement
    LookupError
        Statement Date not found in PG&E bill
        Due Date not found in PG&E bill
        PG&E Account Number not found in bill
    PermissionError
        Failed to copy PDF bill file to output directory
    """
    ev_files: EVFiles = EVFiles(bill_path)

    # Extract text from PDF bill and create Bill instance.

    pdfpages: list[str] = extract_text(
        PDFBEV1(),
        list(divmod(Args.pages, 10)),
        ev_files,
    )  # Extract text pages from BEV-1 bill PDF

    logger.debug(f'Extracted text:\n{"".join(pdfpages)}')

    bill = Bill(ev_files, (pdfpages[0], pdfpages[1]))  # Parse PDF text

    if bill.statement_date:
        try:
            datetime.strptime(bill.statement_date, "%m/%d/%Y")
        except ValueError as e:
            raise ValueError(
                f'PG&E bill Statement Date {bill.statement_date} is not a valid date'
            ) from e

    statement_dt = datetime.strptime(bill.statement_date, '%m/%d/%Y').date()

    if bill.bill_path.stem[4:12] != 'custbill':
        # Rename bill, outdir, and files in outdir to standard format
        # Check bill file name NNNNcustbillMMDDYYYY account number and statement date.
        bill.ev_files.rename(bill.statement_date, bill.account_no)
        bill.bill_path = ev_files.bill_path
        bill.outdir = ev_files.outdir

    outdir: Path = bill.outdir
    bill_file: Path = bill.bill_path
    bill_filename: str = bill_file.name

    bill_pdftxt: Path = bill.ev_files.outdir_path('.txt')
    logger.info(f'Writing {bill_pdftxt} ...')
    with open(bill_pdftxt, 'w', encoding='utf-8') as pdftxt:
        print(bill.as_text(), file=pdftxt)
    ...
    try:
        due_dt = datetime.strptime(bill.due_date, '%m/%d/%Y').date()
    except ValueError as e:
        raise ValueError(f'PG&E bill Due Date {bill.due_date} is not a valid date') from e

    if statement_dt <= bill.period.to:
        warning_msg(
            logger,
            f'Statement Date {bill.statement_date} is not after '
            f'end of billing period {bill.period.to}.',
        )
    elif (statement_dt - bill.period.to) // ONE_DAY > 30:
        warning_msg(
            logger,
            f'Statement Date {bill.statement_date} is more than 30 days '
            f'after end of billing period {bill.period.to}.',
        )

    if due_dt <= statement_dt:
        warning_msg(
            logger,
            f'Due Date {bill.due_date} is not after Statement Date {bill.statement_date}.',
        )
    ...
    if bill.pge_bill_details.period != bill.cpsf_bill_details.period:
        error_msg(
            logger,
            f'PG&E billing period {bill.pge_bill_details.period} '
            f'not equal to CleanPowerSF billing period {bill.cpsf_bill_details.period}.',
        )

    bill.valid()

    # Copy PDF bill file to output directory if no errors found.

    if error_count() == 0:
        dest_file: Path = ev_files.outdir_path('.pdf')
        logger.info(f'Copying "{bill_file}" to "{dest_file}" ...')

        try:
            if not dest_file.exists():  # Copy if file doesn't already exist
                shutil.copy2(bill_file, outdir)
                logger.info(f'File "{bill_file}" copied to "{dest_file}".')
            else:
                info_msg(logger, f'File "{bill_filename}" already exists in "{outdir}"')
        except PermissionError as e:
            raise PermissionError(f'Failed to copy "{bill_path}" to "{dest_file}": {e}') from e

    return bill


def produce_submeter_bills(bill: Bill, energy_monitor: EnergyMonitor) -> list[Submeter]:
    ...
    energy_monitor: EnergyMonitor
        Emporia Vue Energy Monitor instance.
    ...
    """
    bill_path: Path = bill.bill_path
    outdir: Path = bill.outdir
    submeter_bills: list[Submeter] = [
        Submeter(bill, energy_monitor, evse_id)
        for evse_id in sorted(energy_monitor.chargers.keys())
    ]
    ...
    total_submeter_charges: Dollars = sum(
        (subbill.submeter_bill.amount_due for subbill in submeter_bills), Dollars('0.00')
    )
    total_adjustment: Dollars = bill.amount_due - total_submeter_charges

    submeter_total_usage: KilowattHours = sum(
        (subbill.total_kWh for subbill in submeter_bills), KilowattHours('0.00')
    )
    ...
    for nbills, subbill in enumerate(submeter_bills, 1):
        subbill_txt = bill.ev_files.outdir_path(f'-{subbill.evse_id}.txt')
        logger.info(f'Writing {subbill_txt} ...')
        with open(subbill_txt, 'w', encoding='utf-8') as subtxt:

            if bill.total_usage > 0:
                adjustment_share = subbill.total_kWh / submeter_total_usage
                logger.info(f'Adjustment share = ' f'{(adjustment_share):.1%}.')
                subbill.submeter_bill.adjustment = Dollars(total_adjustment * adjustment_share)
            else:
        ...
        subbill_csv: Path = outdir.joinpath(f'{bill_path.stem}-{subbill.evse_id}.csv')
        logger.info(f'Writing {subbill_csv} ...')
        subbill.dump_usage_and_costs(subbill_csv)

        # Write submeter bill PDF file

        subbill_pdf: Path = outdir.joinpath(f'{bill_path.stem}-{subbill.evse_id}.pdf')
        ...
        if error_count() > 0:
            info_msg(logger, f'{error_count()} error(s) found while processing {subbill_pdf}.')

    info_msg(logger, f'{nbills} submeter bills written to "{outdir}".')
...
def main() -> None:
    ...
    RuntimeError
        No PG&E PDF bill files found

    Notes
    -----
    - Extract text from PG&E/CleanPowerSF searchablePDF bills
    ...
    for bill_path in pge_pdf_bill_paths(Args.billfiles):
        ...
        if warning_count() > 0:
            info_msg(logger, f'{warning_count()} warning(s) while processing {bill_path}.')

        if error_count() > 0:
            raise RuntimeError(
                f'{error_count()} error(s) found while processing {bill_path}, '
                f'for details see log file: {Config.evbilling_log}'
            )

        # Produce submeter bills

        evse_file: Path = bill.ev_files.outdir_path('-EVE.json')
        ev_em.discover_chargers(bill.period.to, evse_file)  # type: ignore
        submeter_bills: list[Submeter] = produce_submeter_bills(bill, ev_em)  # type: ignore

        # Write submeter bill total amounts due to NNNNcustbillMMDDYY-DUE.csv file

        due_file: Path = bill.ev_files.outdir_path('-DUE.csv')
        with due_file.open('w', newline='') as csvfile:
            fieldnames = ['EV Charger', 'Amount Due ($)']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            for submeter in submeter_bills:
                writer.writerow(
                    {
                        'EV Charger': submeter.evse_id,
                        'Amount Due ($)': submeter.submeter_bill.total_amount_due,
                    }
                )

        # Write all PDF bill files and NNNcustbillMMDDYY-DUE.csv to NNNNcustbillMMDDYY.zip file

        zip_file: Path = bill.ev_files.outdir_path('.zip')
        ...
        with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            logger.info(f'Adding {due_file.name} to {zip_file} ...')
            zipf.write(due_file, due_file.name)
        info_msg(logger, f'{zip_file} written to "{outdir}".')

    if bill_count == 0:
        raise RuntimeError(
            f'No PG&E PDF bill files found in "{Args.billfiles}"; '
            f'bill files must be named xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx-.pdf '
            f'where x is a digit or letter a-f OR NNNNcustbillMMDDYYYY.pdf '
            f'where NNNN are the 4 digits of the Account Number xxxxxxNNNN-x '
            f'and MMDDYYYY is the Statement Date.'
        )
...
def cli() -> None:
    """Command line interface for evbilling.py."""
    try:
        main()
